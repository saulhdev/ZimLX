...
import org.zimmob.zimlx.groups.AppGroups
import org.zimmob.zimlx.groups.DrawerFolders
import org.zimmob.zimlx.groups.DrawerTabs
import org.zimmob.zimlx.settings.SettingsBottomSheet
import org.zimmob.zimlx.util.applyColor
import org.zimmob.zimlx.util.zimPrefs
import org.zimmob.zimlx.views.BaseBottomSheet
...
class DrawerTabEditBottomSheet(context: Context, config: AppGroups.Group.CustomizationMap,
    ...
    init {
        ...
        val accent = Utilities.getZimPrefs(context).accentColor
    ...
    companion object {
        ...
        fun editTab(launcher: Launcher, group: DrawerTabs.Tab) {
            ...
            edit(launcher, config, group, true) {
                launcher.zimPrefs.drawerTabs.saveToJson()
            }
        }
        ...
        fun editFolder(launcher: Launcher, group: DrawerFolders.Folder) {
            val config = AppGroups.Group.CustomizationMap(group.customizations)
            edit(launcher, config, group, true) {
                launcher.zimPrefs.appGroupsManager.drawerFolders.saveToJson()
            }
        }
