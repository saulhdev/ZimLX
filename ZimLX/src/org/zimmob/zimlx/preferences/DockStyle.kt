...
import org.zimmob.zimlx.util.PropertyDelegate
...
abstract class DockStyle(protected val manager: StyleManager) {
    ...
    private abstract class PredefinedStyle(manager: StyleManager,
        ...
        override var enableGradient
            get() = defaultGradient
            set(value) {
                setProp(gradientProperty, value, defaultGradient)
            }

        override var enableShadow
            get() = defaultShadow
            set(value) {
                setProp(shadowProperty, value, defaultShadow)
            }

        override var radius
            get() = defaultRadius
            set(value) {
                setProp(radiusProperty, value, radius)
            }

        override var opacity
            get() = defaultOpacity
            set(value) {
                setProp(opacityProperty, value, opacity)
            }
        ...
        override var hide
            get() = defaultHide
            set(value) {
                setProp(hideProperty, value, defaultHide)
            }
