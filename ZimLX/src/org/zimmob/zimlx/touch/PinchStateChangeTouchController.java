    ...
    private void onPinchInteractionCompleted(LauncherState launcherState, int i) {
        ...
        if (i2) {
            if (launcherState != mFromState) {
                mLauncher.getUserEventDispatcher().logStateChangeAction(i, getDirectionForLog(), 6, 0,
                        mFromState.containerType, mToState.containerType, mLauncher.getWorkspace().getCurrentPage(), 0);
            }
