...
open class ZimBackup(val context: Context, val uri: Uri?) {
    ...
    class LegacyBackup(context: Context) : ZimBackup(context, null) {

        override fun readMeta(): Meta? {
            var contents = 0
            if (DumbImportExportTask.getDbBackupFile().exists()) {
                contents = contents or ZimBackup.INCLUDE_HOMESCREEN
            }
            if (DumbImportExportTask.getSettingsBackupFile().exists()) {
                contents = contents or ZimBackup.INCLUDE_SETTINGS
            }
    ...
    companion object {
        ...
        fun listLocalBackups(context: Context): List<ZimBackup> {
            val backupList = getFolder().listFiles { file -> file.extension == EXTENSION }
                    ...
                    ?.map { ZimBackup.fromUri(context, it) }
                    ?: Collections.emptyList()
            val legacyBackup = ZimBackup.fromUri(context, null)
        ...
        fun fromUriString(context: Context, uri: String?): ZimBackup {
            return if (uri == null) {
                LegacyBackup(context)
            } else {
                ZimBackup(context, Uri.parse(uri))
            }
        }

        fun fromUri(context: Context, uri: Uri?): ZimBackup {
            return if (uri == null) {
                LegacyBackup(context)
            } else {
                ZimBackup(context, uri)
            }
        }
    }
}
