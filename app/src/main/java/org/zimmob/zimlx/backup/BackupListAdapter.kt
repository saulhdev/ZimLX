...
class BackupListAdapter(val context: Context) : RecyclerView.Adapter<BackupListAdapter.Holder>() {
    ...
    private val backupList = ArrayList<ZimBackup>()
    private val backupMetaLoaderList = ArrayList<ZimBackup.MetaLoader>()

    var callbacks: Callbacks? = null

    fun setData(data: List<ZimBackup>) {
        backupList.clear()
        data.forEach {
            backupList.add(it)
            backupMetaLoaderList.add(ZimBackup.MetaLoader(it))
        }
    }

    fun addItem(backup: ZimBackup) {
        backupList.add(0, backup)
        backupMetaLoaderList.add(0, ZimBackup.MetaLoader(backup))
