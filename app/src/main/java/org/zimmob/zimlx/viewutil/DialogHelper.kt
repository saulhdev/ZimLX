...
import org.zimmob.zimlx.util.copy
import org.zimmob.zimlx.util.getStartAppIntent
import java.io.File
import java.util.*
                ...
                .itemsCallback { dialog, itemView, item, text ->
                    ...
                    if (context.resources.getStringArray(R.array.entries__backup_options)[item] == context.resources.getString(R.string.dialog__backup_app_settings__backup)) {
                        if (isStorageMounted()) {
                            ...
                            Log.i("BACKUP", "Saving Backup at " + directory)
                            if (!directory.exists()) {
                                directory.mkdir()
                            }
                            try {
                            ...
                            val directory = File(context.filesDir.toString())
                            Log.i("BACKUP", "Saving Backup at " + directory)
                            if (!directory.exists()) {
                                directory.mkdir()
                            }
                    ...
                    if (context.resources.getStringArray(R.array.entries__backup_options)[item] == context.resources.getString(R.string.dialog__backup_app_settings__restore)) {
                        val directory = File(context.getExternalFilesDir(null).toString())
                        Log.i("BACKUP", "Reading Backup at " + directory)
                        try {
                            val p = m.getPackageInfo(s, 0)
                            s = p.applicationInfo.dataDir
