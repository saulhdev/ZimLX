package org.zimmob.zimlx.activity.homeparts;

import android.graphics.Point;

import org.zimmob.zimlx.R;
import org.zimmob.zimlx.launcher.Launcher;
import org.zimmob.zimlx.manager.Setup;
import org.zimmob.zimlx.model.Item;
import org.zimmob.zimlx.util.IDialogListener;
import org.zimmob.zimlx.util.Tool;

public class HpDesktopPickAction implements IDialogListener.OnAddAppDrawerItemListener {
    private Launcher _launcher;

    public HpDesktopPickAction(Launcher home) {
        _launcher = home;
    }

    public void onPickDesktopAction() {
        Setup.eventHandler().showPickAction(_launcher, this);
    }

    @Override
    public void onAdd(int type) {
        Point pos = _launcher.getDesktop().getCurrentPage().findFreeSpace();
        if (pos != null) {
            _launcher.getDesktop().addItemToCell(Item.newActionItem(type), pos.x, pos.y);
        } else {
            Tool.toast(_launcher, R.string.toast_not_enough_space);
        }
    }
}
